...
import {
  ...
  User, FileText, Loader2, AlertCircle, ShieldCheck
} from 'lucide-react';
  ...
  return (
    <div className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${cfg.border}`}>
      ...
      <div className="p-5 space-y-4">
        ...
        {/* HOD approval trail */}
        {req.hod_approved_by && (
          <div className="flex items-start gap-2 bg-violet-50 border border-violet-100 rounded-xl px-3 py-2">
            <ShieldCheck className="w-4 h-4 text-violet-500 flex-shrink-0 mt-px" />
            <p className="text-[12px] text-violet-700 font-semibold">HOD approved</p>
          </div>
        )}
...
export const AuthorityLeaveApprovals = () => {
  const [requests, setRequests] = useState([]);
  const [loading, setLoading] = useState(true);
  const [acting, setActing] = useState(null);
  const [filter, setFilter] = useState('active');
  const [error, setError] = useState('');
  // The backend resolves whether this authority is dean or om/principal
  // and returns requests at the appropriate stage.
  // We detect which pending status applies from the first pending item.
  const [pendingStatus, setPendingStatus] = useState('pending_om');
  ...
  const pendingCount = requests.filter(r => r.status === pendingStatus).length;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Final approval stage for faculty leave requests
          </p>
        </div>
        {pendingCount > 0 && (
          <span className="inline-flex items-center gap-1.5 bg-orange-100 text-orange-700 font-bold text-sm px-3 py-1.5 rounded-full">
      ...
      <div className="flex flex-wrap gap-2">
        {[
          { key: 'active', label: 'Awaiting My Approval' },
