...
const OMDashboard = () => {
  ...
  const fetchDashboardStats = async (showRefreshing = false) => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/dashboard/om/analytics`, {
        headers: {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-6 text-left max-w-xl text-[#ffffff]">
            ...
            <div className="space-y-2">
              <h1 className="text-2xl sm:text-3xl font-black tracking-tight leading-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-white/80">
                {greeting.text}, {user?.name || 'Office Manager'}
              </h1>
              ...
              <div className="pt-2">
                <p className="text-white/90 text-[#ffffff]/90 text-xs font-bold uppercase tracking-widest text-indigo-200">
                  OPERATIONS MANAGER OF ACADEMICS
                </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <button onClick={() => navigate('/office manager/messaging')} className="flex items-center gap-3 p-4 bg-white rounded-2xl border border-gray-100 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all group cursor-pointer text-left focus:outline-none">
