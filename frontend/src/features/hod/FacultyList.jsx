...
export const FacultyList = () => {
  ...
  const handleViewProfile = async (facultyMember) => {
    setProfileModal({ open: true, data: null, loading: true });
    try {
      const res = await axios.get(`/api/hod/faculty/${facultyMember.id}/profile`);
          ...
          {error ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100">
                {faculty.map(f => (
                  <tr key={f.id} className="hover:bg-gray-50/50 transition-colors cursor-pointer" onClick={() => handleViewProfile(f)}>
          ...
          <div className="bg-white rounded-2xl sm:rounded-[24px] shadow-2xl max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-100">
              <div className="flex items-center space-x-3 sm:space-x-4">
                ...
                <div className="min-w-0">
                  <h2 className="text-lg sm:text-xl font-bold text-gray-900 truncate">Faculty Profile</h2>
                  {profileModal.data && !profileModal.loading && !profileModal.data.error && (
                    <p className="text-xs sm:text-sm text-gray-500 font-medium truncate">
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6">
              {profileModal.loading ? (
                ...
                <div className="space-y-4 sm:space-y-6">
                  {/* Basic Info */}
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100/50 p-4 sm:p-6 rounded-xl border border-blue-200">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <p className="text-xs font-bold text-blue-600 uppercase tracking-wide mb-1">Employee ID</p>
                        <p className="text-lg font-bold text-blue-900">{profileModal.data.employee_id}</p>
                      </div>
                      <div>
                        <p className="text-xs font-bold text-blue-600 uppercase tracking-wide mb-1">Designation</p>
                        <p className="text-lg font-bold text-blue-900">{profileModal.data.designation || 'Faculty'}</p>
                      </div>
                      <div>
                        <p className="text-xs font-bold text-blue-600 uppercase tracking-wide mb-1">Department</p>
                        <p className="text-lg font-bold text-blue-900">{profileModal.data.department_name}</p>
                      </div>
                      <div>
                        <p className="text-xs font-bold text-blue-600 uppercase tracking-wide mb-1">Date of Joining</p>
                        <p className="text-lg font-bold text-blue-900">{profileModal.data.date_of_joining || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Personal Info */}
                  <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6">
                    <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-4">Personal Information</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
